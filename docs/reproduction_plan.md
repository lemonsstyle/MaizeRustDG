...
基础模型只作为可替换模块，不改变任务定义和外层测试协议：

```text
ConvNeXt / DINO 特征
          +
SAM/PlantSeg 叶片或病斑 ROI
          +
温湿度时序
          ↓
任务适配器与门控融合
          ↓
CORAL 有序分级 + 源域 Deep CORAL
```

多模态大模型可用于候选标签、质量审查和解释实验，但其输出必须经过人工复核，不能直接作为主测试标签。DINOv3、SAM3 等更新模型列为扩展对照；在官方来源、许可证和可用权重未核验前，不纳入主结论。
...
基础模型扩展实验（在 E0–E9 稳定后再做）：

| ID | 视觉 backbone/ROI | 目的 |
|---|---|---|
| F1 | DINOv2 frozen + 有序头 | 检验自监督特征相对监督 CNN 的收益 |
| F2 | DINOv2 + 叶片 ROI | 检验背景抑制效果 |
| F3 | DINOv2 + 病斑 ROI | 检验局部病斑特征效果 |
| F4 | 原图 + ROI 双视图 | 检验全局与局部互补性 |
| F5 | 经官方核验的新版本 backbone | 仅作为可复现扩展，不提前承诺提升 |
