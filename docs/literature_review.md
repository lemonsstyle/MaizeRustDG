...
本课题把近年的基础模型演进纳入研究框架，但采用“辅助—适配—验证”的方式：

| 层次 | 项目中的角色 | 不应做出的过度推断 |
|---|---|---|
| DINO/DINOv2（可扩展到经核验的更新版本） | 冻结或轻量微调的图像表征 backbone | 通用特征不会自动学会本地病情等级标准 |
| SAM/SAM2/PlantSeg | 叶片 ROI、病斑初始掩膜和标注辅助 | 任意物体分割能力不等于锈斑像素级真值 |
| 多模态大模型 | 弱标注、图像质量审查、解释和交互原型 | 生成一个等级文本不能替代可重复的监督评估 |
| 有序头 + 天气编码 + DG | 本论文的任务适配与科学贡献 | 基础模型规模本身不等于跨地点泛化 |

因此，DINO/SAM 等模块应作为有明确对照的实验因素加入，而不是一次性堆叠。新版本或新论文只有在官方来源、权重、许可证和本地 LOSO 结果核验后才能进入主表。
